...
int main()
{
   ...
   int i, j, k, biWidth4, i2, j2;
   double angle = -21.0 * M_PI / 180.0;  // 轉成弧度
   double cosA = cos(angle), sinA = sin(angle);

   i = ReadDataSize("Fig0240(a)(letter_T).bmp");
   data1 = (U_CHAR *)malloc( i );
   if (data1 == NULL) {
      exit(0);
   }
  
   ReadImageData("Fig0240(a)(letter_T).bmp", bmpfileheader1, bmpinfoheader1, color_table1, data1);
   ...
   int centerX = biWidth / 2;
   int centerY = biHeight / 2;
   ...
   for (i = 0; i < biHeight; i++) {
       for (j = 0; j < biWidth; j++) {        
           ...
           // 套用旋轉矩陣
           j2 = (int)(x * cosA - y * sinA) + centerX;
           i2 = (int)(x * sinA + y * cosA) + centerY;

           if (i2 >= 0 && i2 < biHeight && j2 >= 0 && j2 < biWidth) {
               data2[i * biWidth4 + j] = data1[i2 * biWidth4 + j2];
           } else {
               data2[i * biWidth4 + j] = 0; // 邊界填充黑色
           }
       }
   }
