...
int main()
{
   ...
   U_CHAR *data1, *modified_data, color_table1[1024];

 
   INT32 biWidth = 0 ;		
   INT32 biHeight = 0 ;

   int i, j, k, minVal = 255, maxVal = 0;

   i = ReadDataSize("Fig0230(a)(washington_infrared).bmp");
   data1 = (U_CHAR *)malloc( i );
   if (data1 == NULL) {
      exit(0);
   }
   
   modified_data = (U_CHAR *)malloc( i );
   if (modified_data == NULL) {
      exit(0);
   }
  
   ReadImageData("Fig0230(a)(washington_infrared).bmp", bmpfileheader1, bmpinfoheader1, color_table1, data1);
   ...
   for (i = 0; i < biHeight; i++) {
       k = i * ((biWidth * 1 + 3) / 4 * 4);
       for (j = 0; j < biWidth; j++) {
           modified_data[k] = data1[k] & 0xFE; // 清除 LSB
           data1[k] = data1[k] - modified_data[k]; // 計算差值
           if (data1[k] < minVal) minVal = data1[k];
           if (data1[k] > maxVal) maxVal = data1[k];
           k++;
//           printf("%d", data1[k]);
       }
   }
   ...
   for (i = 0; i < biHeight; i++) {
       k = i * ((biWidth * 1 + 3) / 4 * 4);
       for (j = 0; j < biWidth; j++) {
           data1[k] = (U_CHAR)(((data1[k] - minVal) * 255) / (maxVal - minVal));
           k++;
       }
   }
