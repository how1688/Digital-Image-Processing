...
#define TRI_TOP_X 500
#define TRI_TOP_Y 1024-300
#define TRI_SIDE 400
...
int main()
{
    ...
    memcpy(data2, data1, i);

    baseY = TRI_TOP_Y - (int)(TRI_SIDE * sqrt(3) / 2);
    baseX1 = TRI_TOP_X - TRI_SIDE / 2;
    baseX2 = TRI_TOP_X + TRI_SIDE / 2;

    for (i = TRI_TOP_Y; i >= baseY; i--)
    {
        int leftX = TRI_TOP_X - (int)((TRI_TOP_Y-i) / slope);
        int rightX = TRI_TOP_X + (int)((TRI_TOP_Y-i) / slope);
        k = i * biWidth;
    ...
    if ((output_file = fopen("hw1.bmp", "wb")) == NULL)
    {
