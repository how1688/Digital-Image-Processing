...
int main()
{
   ...
   //
   new_data = (U_CHAR *)malloc( NEW_WIDTH*NEW_HEIGHT*1 );
   if (new_data == NULL) {
      fprintf(stderr,"Insufficient memory.\n");
      exit(0);
   }
   ...
   for (int i = 0; i < NEW_HEIGHT; i++) {
       for (int j = 0; j < NEW_WIDTH; j++) {
           //設定變數
           int src_x = (int)((j * biWidth) / (double)NEW_WIDTH);
           int src_y = (int)((i * biHeight) / (double)NEW_HEIGHT);
           double x_diff = (j * biWidth) / (double)NEW_WIDTH - src_x;
           double y_diff = (i * biHeight) / (double)NEW_HEIGHT - src_y;
           //設定四個點
           int p1 = data[src_y * biWidth + src_x];
           int p2 = data[src_y * biWidth + src_x + 1];
           int p3 = data[(src_y + 1) * biWidth + src_x];
           int p4 = data[(src_y + 1) * biWidth + src_x + 1];
           //雙線性內插f(x,y)
           int interpolated_value = (int)(p1 * (1 - x_diff) * (1 - y_diff) +
               p2 * x_diff * (1 - y_diff) +
               p3 * (1 - x_diff) * y_diff +
               p4 * x_diff * y_diff);

           new_data[i * NEW_WIDTH + j] = (U_CHAR)interpolated_value;
       }
   }
   ...
   set_4B(new_bmpfileheader, 2, NEW_WIDTH*NEW_HEIGHT + 1078);
   set_4B(new_bmpinfoheader, 4, NEW_WIDTH);
   set_4B(new_bmpinfoheader, 8, NEW_HEIGHT);
   set_4B(new_bmpinfoheader, 20, NEW_WIDTH*NEW_HEIGHT);
   ...
   fwrite(new_data, NEW_WIDTH * NEW_HEIGHT*1, 1, output_file);
