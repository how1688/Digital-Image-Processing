...
int main()
{
    ...
    //讀取原始圖片
    i = ReadDataSize("pic2.bmp");
    data1 = (U_CHAR*)malloc(i);
    if (data1 == NULL) {
        exit(0);
    }

    ReadImageData("pic2.bmp", bmpfileheader1, bmpinfoheader1, color_table1, data1);
    ...
    //生成第二張圖data2[pixel]:原始圖片的data[pixel-1]
    for (i = 0; i < biHeight; i++)
    {
        k = i * ((biWidth * 1 + 3) / 4 * 4);
        for (j = 0; j < biWidth; j++)
        {
            data2[k] = (data1[k] % 2 == 0) ? data1[k] : (data1[k] - 1);
            k = k + 1;
        }
    }
    //生成新圖片(new2)
    ...
    //生成第三張圖data1[pixel]:原始圖片的data1[pixel]-data2[pixel]
    for (i = 0; i < biHeight; i++)
    {
        k = i * ((biWidth * 1 + 3) / 4 * 4);
        for (j = 0; j < biWidth; j++)
