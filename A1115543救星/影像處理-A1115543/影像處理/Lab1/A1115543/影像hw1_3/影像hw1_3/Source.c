...
int main()
{
    ...
    for (i = 0; i < biHeight; i++)
    {
        ...
        for (j = 0; j < biWidth; j++)
        {
            if (data1[k] <= 255 && data1[k] > 127) { data2[k] = 255; }    //當灰階介於128~255之間，一律取255
            else if (data1[k] <= 127 && data1[k] > 85) { data2[k] = 127; }//當灰階介於 86~255之間，一律取127
            else { data2[k] = 0; }//其餘一律取0
            k = k + 1;
        }
    }
    ...
    //
    /* 開啟新檔案 */
    if ((output_file = fopen("three_level.bmp", "wb")) == NULL) {
