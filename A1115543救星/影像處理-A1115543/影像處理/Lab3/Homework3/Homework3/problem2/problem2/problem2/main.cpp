...
int main()
{
	...
	for (i = 0; i < biHeight; i++) {
		...
		for (i1 = i - a; i1 <= i + a; i1++) {
			...
			for (j1 = 0 - a; j1 <= a; j1++) {
				...
				if (data1[ni*biWidth4 + nj] < 0) dataArray[mi][mj] = 0;
				else if (data1[ni*biWidth4 + nj] > 255) dataArray[mi][mj] = 255;
				else dataArray[mi][mj] = data1[ni*biWidth4 + nj];
				sum += dataArray[mi][mj];
				mj++;
			}
			mi++;
		...
		for (j = 1; j < biWidth; j++) {
			...
			for (i1 = i - a; i1 <= i + a; i1++) {
				correctedPos(&ni, &nj, i1, j1, biHeight, biWidth);
				sum += dataArray[mi][idx_col];
				if (data1[ni*biWidth4 + nj] < 0) dataArray[mi][idx_col] = 0;
				else if (data1[ni*biWidth4 + nj] > 255) dataArray[mi][idx_col] = 255;
				else dataArray[mi][idx_col] = data1[ni*biWidth4 + nj];
				sum += dataArray[mi][idx_col];
				mi++;
			}
			data2[k] = sum / size_square;
	...
	//
	/* 開啟新檔案 */
	if ((output_file = fopen("bb.bmp", "wb")) == NULL) {
