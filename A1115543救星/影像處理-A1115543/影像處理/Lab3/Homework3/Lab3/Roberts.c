...
int main()
{
   ...
   i = ReadDataSize("Fig0342(a).bmp");
   data1 = (U_CHAR *)malloc( i );
   if (data1 == NULL) {
      exit(0);
   }
  
   ReadImageData("Fig0342(a).bmp", bmpfileheader1, bmpinfoheader1, color_table1, data1);
   ...
   for (i=0; i < biHeight-1; i++)
   {
       ...
       for (j=0; j < biWidth-1; j++)
       {           
		   temp = data1[k+biWidth4+1] - data1[k];
           temp = (temp < 0)? -temp: temp; 
           sum = temp;
		   //
           temp = data1[k+biWidth4] - data1[k+1];
           temp = (temp < 0)? -temp: temp; 
           sum = sum + temp;
           //
           data2[k] = sum;
   ...
   //
   /* 開啟新檔案 */
   if( ( output_file = fopen("roberts.bmp","wb") ) == NULL ){
