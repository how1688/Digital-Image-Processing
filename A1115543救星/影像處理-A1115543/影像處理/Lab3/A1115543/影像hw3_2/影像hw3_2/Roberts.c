...
int main()
{
   ...
   //若完成(a)，要將(b)(c)當作註解，並將輸出file名稱改成(a)Laplacian.bmp
  //(a)Laplacian.bmp
   for (i = 0; i < biHeight - 1; i++)
   {
	   k = i * biWidth4;
   ...
   //若執行(b)，要將(c)當作註解，並將輸出file名稱改成(b)sharpen.bmp
   //(b)sharpen.bmp
   /*for (i = 0; i < biHeight - 1; i++)
   {
	   k = i * biWidth4;
	   ...
	   {   //g(x,y)= f(x,y)-Laplacian
		   data2[k] = data1[k]-data2[k]-128;//-128是為了讓圖更接近目標的樣子

		   if (data2[k] > 255) { data2[k] = 255; }
		  
		   k = k + 1;
	   }
   }*/

   //若執行(c)，要將(a)(b)當作註解，並將輸出file名稱改成(c)sharpen2.bmp
   //(c)sharpen2.bmp
   /*
   for (i = 0; i < biHeight - 1; i++)
   {
	   k = i * biWidth4;
	   for (j = 0; j < biWidth - 1; j++)
	   {   //g(x,y) = (-8)*f(x,y)+f(x-1,y)+f(x+1,y)+f(x,y+1)+f(x,y-1)
		   data2[k] = (-8 * data1[k] + data1[k - 1] + data1[k + 1] + data1[k + biWidth4] + data1[k - biWidth4]) -128;//-128為了讓圖更接近目標的樣子
		   if (data2[k] < 0) { data2[k] = 0; }
		   k = k + 1;
	   }
   }
   ...
   //
   /* 開啟新檔案 */
   if( ( output_file = fopen("(a)Laplacian.bmp","wb") ) == NULL ){
