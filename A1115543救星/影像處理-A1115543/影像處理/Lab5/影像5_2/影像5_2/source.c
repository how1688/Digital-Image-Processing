...
int main()
{
    ...
    U_CHAR* data1, * data2, * data3, * data4, * data5, color_table1[1024];
    COMPLEX* c, * c2;
    ...
    int i, j, k, temp, temp2, biWidth4;
    int histo_table[256] = { 0 };
    double max, D, H;
    int D0 = 15, n = 2;


    i = ReadDataSize("Fig0464(a).bmp");
    ...
    data4 = (U_CHAR*)malloc(i);
    if (data4 == NULL) {
        exit(0);
    }

    data5 = (U_CHAR*)malloc(i);
    if (data5 == NULL) {
        exit(0);
    }

    ReadImageData("Fig0464(a).bmp", bmpfileheader1, bmpinfoheader1, color_table1, data1);
    ...
    for (i = 0; i < biHeight; i++)
    {
        ...
        for (j = 0; j < biWidth; j++)
        {   //截止頻率皆設為10
            //右上1 根號( i-(biheight/2+45))^2+ (j-(biwidth/2+45))^2 )
            D = sqrt((i - biHeight / 2 - center_x) * (i - biHeight / 2 - center_x) + (j - biWidth / 2 - center_y) * (j - biWidth / 2 - center_y));
            if (D <= 10)
            {
                c2[k].real = c[k].real;
                c2[k].imag = c[k].imag;
            ...
            //左下1 根號( i-(biheight/2-45))^2+ (j-(biwidth/2-45))^2 )
            D = sqrt((i - biHeight / 2 + center_x) * (i - biHeight / 2 + center_x) + (j - biWidth / 2 + center_y) * (j - biWidth / 2 + center_y));
            if (D <= 10)
            {
                c2[k].real = c[k].real;
            ...
            //左上1 根號( i-(biheight/2+45))^2+ (j-(biwidth/2-245))^2 )
            D = sqrt((i - biHeight / 2 - center_x) * (i - biHeight / 2 - center_x) + (j - biWidth / 2 + center_y) * (j - biWidth / 2 + center_y));
            if (D <= 10)
            {
                c2[k].real = c[k].real;
            ...
            //右下1 根號( i-(biheight/2-45))^2+ (j-(biwidth/2+45))^2 )
            D = sqrt((i - biHeight / 2 + center_x) * (i - biHeight / 2 + center_x) + (j - biWidth / 2 - center_y) * (j - biWidth / 2 - center_y));
            if (D <= 10)
            {
                c2[k].real = c[k].real;
            ...
            //第二對(較外側)
            //右上2 (center_x較第一對大2倍)
            D = sqrt((i - biHeight / 2 - center_x*2) * (i - biHeight / 2 - center_x*2) + (j - biWidth / 2 - center_y) * (j - biWidth / 2 - center_y));
            if (D <= 10)
            {
                c2[k].real = c[k].real;
                c2[k].imag = c[k].imag;
    ...
    // inverse Fourier transform, c2
    if (!FFT2D(c2, biHeight, biWidth, -1))
    {
        printf("Stop!\n");
        exit(0);
    }
    ...
    // c
    /* 開啟新檔案 */
    if ((output_file = fopen("new4.bmp", "wb")) == NULL) {
    ...
    fwrite(data4, ((biWidth * 1 + 3) / 4 * 4) * biHeight * 1, 1, output_file);

    fclose(output_file);

    // c2
    /* 開啟新檔案 */
    if ((output_file = fopen("noise.bmp", "wb")) == NULL) {
        fprintf(stderr, "Output file can't open.\n");
        exit(0);
    }
    ...
    fwrite(data5, ((biWidth * 1 + 3) / 4 * 4) * biHeight * 1, 1, output_file);
